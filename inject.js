...
function compileMatchSelectors() {
  var selectors = new Map();
  selectors.set("body.deviantart", [".gr > .metadata > h2", ".gr > .metadata > ul > li.author", ".gr-body > .gr > .grf-indent"]);
...
var matchSelectors = compileMatchSelectors();
...
function findSelectors() {
  for(var [matchSelector, nodeSelectors] of matchSelectors) {
    if(document.querySelector(matchSelector)) return nodeSelectors;
  }
}

function getText(selectors) {
  var nodes = selectors.map(function(selector) {
